backend
+npm init -y
/npm install express mongoose cors dotenv
/npm install nodemon --save-dev
/create .env MONGO_URI= 0.0.0.0/0 PORT=5000
	"start": "node server.js",
  	"dev": "nodemon server.js"
    
rool folder
+npm create vite@latest frontend
frontend - npm install axios
+.env - VITE_API_URL=http://localhost:5000/api
replace app.jsx

git init/git add./git commit -m ""/
git remote add origin repolink
git branch -M main/git push -u origin main

backend-render-new.webservice/branch-main/root-backend/Build Command-npm install
/Start Command-npm start (or node server.js)
advanced-Environment Variables-MONGODB_URI = your connection string/PORT = 5000
/'Create Web Service'/copy render url to frntend .env/

frontend deploy-Vercel/Framework Preset: Vite/Build Command: npm run build
/Output Directory: dist/environment variable vitapi-url=render url
netlify same.frontend/dist-- functiondirectory-frntend/
